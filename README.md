...
##### Настройка окружения и запуск сервиса
1) Python: убедитесь, что на вашем компьютере установлен Python 3.7 или новее.
2) Установка библиотек: установите библиотеки с помощью следующей команды: `pip install -r requirements.txt` 
3) Запуск приложения: выполняется командой `streamlit run streamlit_map.py`

После выполнения этих шагов вы сможете запустить приложение, открыв браузер и перейдя по адресу http://localhost:8501 (или другому порту, указанному в выводе команды streamlit run).
